...
function buildingMesh(way, nodeById, pois = []) {
    ...
    try {
        ...
        if (buildingType && closestPoi) {
            mesh.userData.isPoiBuilding = true;
            mesh.userData.poiType       = buildingType;
            mesh.userData.tags          = { ...closestPoi.tags, _lat: closestPoi.lat, _lon: closestPoi.lon };
        }
        if (buildingType=='confirmed'||buildingType=='suspected'){mesh.position.y+=1;};
        mesh.userData.tags = (buildingType && closestPoi)
            ? { ...closestPoi.tags, _lat: closestPoi.lat, _lon: closestPoi.lon }
            : tags;
        return mesh;
    } catch {
        return null;
...
class Tile {
    ...
    _clearMeshes() {
        ...
        for (const obj of toRemove) {
            ...
            if (obj.userData?.isPoiBuilding) {
                _poiBuildingSet.delete(obj);
                if (obj === selectedPOI) selectedPOI = null;
                if (obj === hoveredPOI)  hoveredPOI  = null;
            }
        }
    ...
    _buildMeshes(data, pois) {
        ...
        const poiInfo = [
            ...activeSuspected.map(p => ({
                ...toWorld(p.lat, p.lon),
                type: 'suspected',
                lat: p.lat, lon: p.lon, tags: p.tags ?? {},
            })),
            ...allConfirmed.map(p => ({
                ...toWorld(p.lat, p.lon),
                type: 'confirmed',
        ...
        for (const el of data.elements) {
            if (el.type === 'way' && el.tags?.building) {
                ...
                if (m) {
                    this.group.add(m);
                    if (m.userData.isPoiBuilding) _poiBuildingSet.add(m);
                }
            }
        }
...
function hitPOI(clientX, clientY) {
    ...
    const hits = raycaster.intersectObjects([..._poiMeshSet, ..._poiBuildingSet]);
